...
public class CoinMove : MonoBehaviour
{
    ...
    private void Awake()
    {
        // RectTransform을 인스펙터에서 직접 연결하지 않고, 스크립트를 통해 연결하는 경우 추가하는 코드
        ...
        while (true)
        {
            for (int i = 0; i < rects.Length; i++)
            {
                ...
                rect.position = Vector2.MoveTowards(rect.position, 
                                B_Canvas.instance.Coin.position, item_move_speed * Time.deltaTime);
            }
    ...
    private bool CheckDistance(Vector2[] end, float range)
    {
        ...
        for (int i = 0; i < rects.Length; i++)
        {
            ...
            // a 와 b 사이 거리를 체크하는 문법 Vector2.Distance(Vector2 a, Vector2 b);
