...
public class Attack : MonoBehaviour
{
    ...
    public void Init(Transform t, double dmg, string key)
    {
        ...
        attacks[key].gameObject.SetActive(true);
    }

    private void Update()
    {
        // hit 가 활성화된 상태라면 return 해서 hit 에 대한 처리가 다 끝난 뒤,
        ...
        if (Vector3.Distance(transform.position, target_pos) <= 0.1f)
        {
            ...
            if (target != null)
            {
                ...
                // 파티클의 duration 은 현재 main.duration 으로 접근 가능합니다.
                // 일반적으로 코루틴은 Start 영역에서 진행하나,
                // Update 여도 특정 조건일 때만 실행되는 수준에서는 사용하는 경우도 있다.
            }
        }
    }
